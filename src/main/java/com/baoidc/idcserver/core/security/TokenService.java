...
public class TokenService implements  AuthorizationServerTokenServices,
	...
	private int refreshTokenValiditySeconds = 2592000; // refresh_token 的超时时间
														// 默认2592000秒
	private int accessTokenValiditySeconds = 10; // access_token 的超时时间 默认12个小时
	private boolean supportRefreshToken = false; // 是否支持access_token
													// 刷新，默认是false,在配置文件中以配置成可以支持了，
	...
	public OAuth2AccessToken createAccessToken(
		...
		OAuth2AccessToken existingAccessToken = this.tokenStore
		...
		if (refreshToken == null) {
			...
			if (System.currentTimeMillis() > accessToken.getExpiration()
					.getTime()) {
				refreshToken = this.createRefreshToken(authentication);
			}
		}

		OAuth2AccessToken accessToken1 = this.createAccessToken(authentication,
	...
	public OAuth2AccessToken refreshAccessToken(String refreshTokenValue,
		...
		if (!this.supportRefreshToken) {
			...
			Object refreshToken = this.tokenStore
			...
			if (refreshToken == null) {
				...
				String clientId = authentication.getOAuth2Request()
						.getClientId();
				if (clientId != null
						&& clientId.equals(tokenRequest.getClientId())) {
					this.tokenStore
					...
					if (this.isExpired((OAuth2RefreshToken) refreshToken)) {
						...
						authentication = this.createRefreshedAuthentication(
								authentication, tokenRequest.getScope());
	...
	private OAuth2Authentication createRefreshedAuthentication(
			OAuth2Authentication authentication, Set scope) {
		...
		if (accessToken == null) {
			...
			if (this.clientDetailsService != null) {
				...
				try {
					...
					throw new InvalidTokenException("Client not valid: "
							+ clientId, var6); // ---------------------客户端信息不正确
												// ，client-id，secret_1等
				}
			}
	...
	public boolean revokeToken(String tokenValue) {
		...
		if (accessToken == null) {
			...
			if (accessToken.getRefreshToken() != null) {
				this.tokenStore.removeRefreshToken(accessToken
						.getRefreshToken());
			}
	...
	private ExpiringOAuth2RefreshToken createRefreshToken(
		...
		if (!this.isSupportRefreshToken(authentication.getOAuth2Request())) {
			...
			String access_tokens = UUID.randomUUID().toString()
					.replaceAll("-", "");
			System.out.println("\n\n-----------------------------------\n\n\n");
			System.out
					.println("-----------------------------------refreshAccess_token = "
							+ access_tokens);
			System.out.println("\n\n-----------------------------------\n\n\n");
			DefaultExpiringOAuth2RefreshToken refreshToken = new DefaultExpiringOAuth2RefreshToken(
					access_tokens, new Date(System.currentTimeMillis()
	...
	private OAuth2AccessToken createAccessToken(
		...
		DefaultOAuth2AccessToken token = new DefaultOAuth2AccessToken(
				access_tokens);
		int validitySeconds = this.getAccessTokenValiditySeconds(authentication
				.getOAuth2Request());
		...
		return this.accessTokenEnhancer != null ? this.accessTokenEnhancer
				.enhance(token, authentication) : token;
	}

	protected int getAccessTokenValiditySeconds(OAuth2Request clientAuth) {
		if (this.clientDetailsService != null) {
			ClientDetails client = this.clientDetailsService
